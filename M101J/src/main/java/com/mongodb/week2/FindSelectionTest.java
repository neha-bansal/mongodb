...
public class FindSelectionTest {
	...
	public static void main(String[] args) {
		...
		//projections
		Document projection = new Document("age",1).append("_id", 0);
		
		Bson projection1 = Projections.fields(Projections.include("profession"),  Projections.excludeId());//Projections.include("name");
		
		System.out.println("Find all with into:");
		List<Document> docList = crudTest.find(filter).projection(projection).into(new ArrayList<Document>());
		for (Document d : docList) {
			Helpers.printJson(d, true);
		}
		System.out.println("================= Find all with iteration:");
		MongoCursor<Document> cursor = crudTest.find(filter).projection(projection1).iterator();
		try {
